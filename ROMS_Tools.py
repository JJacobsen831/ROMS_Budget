...
def ROMS_Load(RomsFile, VarName, latbounds, lonbounds) :
    ...
    lat_li = np.argmin(np.abs(RomsNC.variables['lat_rho'][:] - latbounds[0]))
    lat_ui = np.argmin(np.abs(RomsNC.variables['lat_rho'][:] - latbounds[1]))
    lon_li = np.argmin(np.abs(RomsNC.variables['lon_rho'][:] - lonbounds[0]))
    lon_ui = np.argmin(np.abs(RomsNC.variables['lon_rho'][:] - latbounds[1]))
    
    #locate psi points within bounds
    latpsi_li = np.argmin(np.abs(RomsNC.variables['lat_psi'][:] - latbounds[0]))
    latpsi_ui = np.argmin(np.abs(RomsNC.variables['lat_psi'][:] - latbounds[1]))
    lonpsi_li = np.argmin(np.abs(RomsNC.variables['lon_psi'][:] - lonbounds[0]))
    lonpsi_ui = np.argmin(np.abs(RomsNC.variables['lon_psi'][:] - latbounds[1]))
